...
AboutDlgProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	switch (uMsg)
	{
		...
		SetDlgItemText(hDlg, IDC_VERSION, szWindowText[0]);
	...
	switch (uMsg)
	{
		...
		switch (LOWORD(wParam))
		{
			...
			if (!Profile.logfile_use_date)
			{
				...
				if (_stricmp(szFileLog, Profile.filterfile) == 0)
				{
...
GeneralOptionsDlgProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	switch (uMsg)
	{
		...
		if (_stricmp(Profile.LogfilePath, szLogPathName) == 0) // PH: If default
		{
		...
		switch (LOWORD(wParam))
		{
			...
			SetDlgItemText(hDlg, IDC_LOGFILEPATHTEXT, (_stricmp(tmp_path, szLogPathName) == 0) ? "Path for logfiles (default)" : "Path for logfiles");
			EnableWindow(GetDlgItem(hDlg, IDC_LOGFILEPATHDEFAULT), _stricmp(tmp_path, szLogPathName));
...
ScrollDlgProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	switch (uMsg)
	{
		...
		switch (LOWORD(wParam))
		{
			...
			{
				sprintf(tbuf, "Please enter a percentage between 10 and 90!");	//FVK ,PANE1_SIZE
				MessageBox(hDlg, tbuf, "PDW Scrollback", MB_ICONERROR);
	...
	switch (uMsg)
	{
		...
		switch (LOWORD(wParam))
		{
			...
			if (GetOpenFileName(&ofn))
			{
				...
				if (_stricmp(Profile.LogfilePath, szPathname) == 0)
				{
...
//							if (atoi(filter.capcode) < 2101249) memmove(filter.capcode, &filter.capcode[2], strlen(filter.capcode));
			...
			if (IsDlgButtonChecked(hDlg, IDC_SEPFILTERFILEEN) == BST_CHECKED)
			{
				...
				for (int j=0; j<3; j++)
				{
					...
					{
						if ((_stricmp(tmp_sepfile[0], tmp_sepfile[1]) == 0) && (strcmp(tmp_sepfile[0], "Don't change")))
						{
					...
					{
						if ((_stricmp(tmp_sepfile[0], tmp_sepfile[2]) == 0) && (strcmp(tmp_sepfile[0], "Don't change")))
						{
							MessageBox(hDlg,"Separate filterfiles 1 and 3 are equal.\nPlease change one of them.","PDW Separate Filterfile",MB_ICONERROR);
							SetFocus(GetDlgItem(hDlg, IDC_SEPFILTERFILEBROWSE3));
							return (FALSE);
						}
						if ((_stricmp(tmp_sepfile[1], tmp_sepfile[2]) == 0) && (strcmp(tmp_sepfile[1], "Don't change")))
						{
...
TestIfBigger(int nHigher, int nLower, int bAddress)
{
	...
	if (bAddress)
	{
		...
		if (Profile.filters[nLower].text[0] == '^')
		{
			...
			{
				return(_stricmp(Profile.filters[nHigher].text, Profile.filters[nLower].text) < 0);
			}
			return (1);
		}
		if (Profile.filters[nHigher].text[0] == '^')
		...
		if (_strnicmp(&Profile.filters[nLower].text[0], Profile.filters[nHigher].text, strlen(Profile.filters[nHigher].text)) == 0)
		{
			return (0);
		}
		if (_strnicmp(&Profile.filters[nHigher].text[0], Profile.filters[nLower].text, strlen(Profile.filters[nLower].text)) == 0)
		{
			return (1);
		}
		return(_stricmp(Profile.filters[nHigher].text, Profile.filters[nLower].text) < 0);
	}
	else return(_stricmp(Profile.filters[nHigher].label, Profile.filters[nLower].label) < 0);
}
...
SystemTrayIcon(int bRemoveIcon)
{
	...
	strcpy(nid.szTip, szWindowText[0]);
