...
// 0-12. Max positions available to signal indicator.
#define MAX_SI_POS        20

// Used for working out samples per signal position.e.g. sample=20/10, position=2.
#define AUDIO_POINT_VALUE 2


static HDC hdcMemory = NULL;
static HBITMAP hbm_sigind = NULL;
static BITMAP bms;
static RECT old_rect;     // used for redrawing signal indicator bitmap
static RECT sig_rect;     // used for holding current position of sigind bitmap

static int got_sigind = FALSE;
static int old_rect_flg = FALSE;

// Keep track of signal indicator.
static int si_index = 0;
static int si_old_index = 0;
static int delay_cnt = 0;
static int si_low_hover = 0;
static int si_hi_hover = 0;


// Points used for drawing signal indicator.
static const int sip[21][4] = {
//from: x,y, To: x,y
    { 18, 20,  6, 14 },
//  { 18, 20,  7, 13 },
    { 18, 20,  8, 12 },
...
static void
show_sigind(int new_pos, int old_pos)
{
    HDC hdc = GetDC(ghWnd);
    int x, y;
...
int
LoadSigInd(HINSTANCE hThisInstance)
{
    if (! got_sigind) {
	if (hbm_sigind = LoadBitmap(hThisInstance,MAKEINTRESOURCE((WORD)IDS_SIGIND))) {
		if (GetObject(hbm_sigind, sizeof(bms), &bms))
			got_sigind = TRUE;
		else
			DeleteObject(hbm_sigind);
	}
    }
...
void
UpdateSigInd(int direction_flg)
{
    si_old_index = si_index;

    if (old_rect_flg) {
	if (direction_flg) {
		// Move indictor right 1
		si_low_hover = 0;
